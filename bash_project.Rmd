...
To begin, you will need to run a file called `firstStep.R` that has been attached. This file will download the necessary package, which is called `stringr` and it will create the directory I was discussing in the previous paragraph. Within this directory, five subdirectories will be created along with a lot of random files within them. This is the only file that you will need to download to follow along with this tutorial, yet results may vary because the files created are made at random. 

After you run the R script file called `firstStep.R`, you should see a new directory that has been created in your home directory called `mydir`. This is the directory that we will be using. The first command in all chunks will be changing the directory, and then the code to complete the process will be next.

...
Within this directory that has been created, five subdirectories will be created along with a lot of random files within them. This is the only code that you will need to run to follow along with this tutorial, and we should all get the same results since a seed has been set.
...
Since there is only one line, and one word in this file, that is all that it prints. I typed "hello" in this file, and the `cat` function returns the input. If you would like, you can try this on your own machine by typing something into a file, and use the `cat` command followed by the filename to return what you typed. 
